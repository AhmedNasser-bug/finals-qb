...
The objective is to distill the concrete, session-specific actions (e.g., debugging a specific variable, editing a specific file) into a generalized, robust, and beautiful workflow stored in `.agents/workflows/<workflow_name>.md`.
...
## 4. Workflow Document Structure (`.agents/workflows/<workflow_name>.md`)

Every generated workflow MUST be stored inside `.agents/workflows/` and adhere to the premium, neo-brutalist monospace data design. It must follow this exact markdown template:
...
1. Save the workflow file using the kebab-case or snake_case format: `.agents/workflows/<workflow_name>.md`.
2. Update the main project documentation (e.g. [AGENTS.md](file:///d:/Study/Programming/Projects/finalsv2/finals-qb/AGENTS.md) or a central `.agents/workflows/README.md` catalog if it exists) to list the newly added workflow under the **Available Workflows** list.
