...
import { ShareIcon, StatPill } from "./subject-selector-components"
...
export function UserSubjectCard({
  ...
  return (
    ...
    >
      {/* Absolute overlay for primary action (select) to avoid nesting buttons */}
      <div
        role="button"
        tabIndex={0}
        onClick={() => {
      ...
      <div className="relative z-10 flex items-center justify-between border-t border-border px-4 py-2.5 pointer-events-none">
        ...
        {isConfirming ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-[10px] font-mono font-semibold px-2 py-0.5 border border-destructive/50 text-destructive hover:bg-destructive/10 transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-destructive pointer-events-auto"
            >Yes</button>
            <button
              onClick={onDeleteCancel}
              aria-label={`Cancel deletion of ${full.name}`}
              className="text-[10px] font-mono px-2 py-0.5 border border-border text-muted-foreground hover:text-foreground transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring pointer-events-auto"
            >No</button>
          </div>
        ) : (
          <button
            onClick={(e) => { e.stopPropagation(); onRemoveClick(full.id) }}
            className="text-[10px] font-mono text-muted-foreground/40 hover:text-destructive transition-colors px-1 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-destructive pointer-events-auto"
