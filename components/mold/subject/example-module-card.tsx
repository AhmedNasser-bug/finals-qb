...
import {
  ...
  SpinnerIcon,
  StatPill,
} from "@/components/mold/subject/subject-selector-components";
...
export function ExampleModuleCard({
  ...
  return (
    <div className="group relative flex flex-col bg-panel border border-border hover:border-border/80 transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-[#121318]/50 focus-within:ring-2 focus-within:ring-primary/60 focus-within:ring-offset-2 focus-within:ring-offset-background">
      ...
      <div className="flex flex-col gap-5 p-6 text-left flex-1 relative z-0">
        <div className="flex items-start justify-between gap-3">
          <p className="text-base font-semibold text-foreground tracking-tight leading-snug text-pretty">
            {entry.name}
          </p>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          ...
          {entry.tags.map((tag) => (
            <span
              key={tag}
              className="text-[10px] font-mono px-2 py-0.5 border border-border bg-background text-muted-foreground"
            >
              {tag}
            </span>
          ))}
