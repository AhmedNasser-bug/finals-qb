...
interface RevenueCatLogoProps extends React.SVGProps<SVGSVGElement> {
  className?: string
  color?: string
}

/**
 * Official RevenueCat Brand Mark SVG
 * Signature Brand Coral: #F2545B
 */
export function RevenueCatLogo({
  className = "w-4 h-4",
  color = "#F2545B",
...
/**
 * High-visibility "Made with RevenueCat" brand recognition badge
 */
export function RevenueCatBadge({
  className = "",
  label = "MADE WITH REVENUECAT",
}: {
  className?: string
  label?: string
}) {
  return (
    <a
      ...
      className={`inline-flex items-center gap-2 px-3 py-1.5 border border-[#F2545B]/40 bg-[#F2545B]/10 hover:bg-[#F2545B]/20 transition-all rounded font-mono text-[10px] tracking-widest text-[#F2545B] font-bold uppercase shadow-[0_0_12px_rgba(242,84,91,0.18)] group ${className}`}
      title="RevenueCat In-App Subscriptions and Web Billing"
    >
      <RevenueCatLogo className="w-3.5 h-3.5 shrink-0 transition-transform group-hover:scale-110" />
