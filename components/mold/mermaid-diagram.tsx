...
export function MermaidDiagram({ chart, id, className }: MermaidDiagramProps) {
  ...
  if (error) {
    return (
      <div
        className={`mermaid-container min-h-[200px] bg-[#1a0c0c] border border-[#930013]/50 p-4 space-y-3 ${className ?? ""}`}
        role="alert"
      ...
      >
        ...
        <details>
          ...
          <pre className="mt-2 font-mono text-[11px] bg-[#0e0e0e] border border-[#353534] p-3 overflow-x-auto text-[#fecc17]/70 leading-relaxed">
            {chart}
          </pre>
        </details>
      </div>
