...
export function Step1ContentType({
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          return (
            ...
            >
              <span className={cn("text-sm font-mono font-bold uppercase tracking-wide", isSelected ? "text-primary" : "text-white")}>
                {type.name}
              </span>
              <span className="text-[11px] leading-snug font-sans text-zinc-500">
                {type.desc}
              </span>
            </button>
          )
      ...
      {/* Volume Configurator */}
      <div className="p-5 border border-border bg-[#101115] space-y-5 rounded-none mt-4">
        <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider border-b border-border/50 pb-2">
          Target Generation Volume
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {(contentType === "questions" || contentType === "both") && (
            <div className="space-y-3">
              <label className="text-[11px] font-mono font-bold tracking-widest text-zinc-400 uppercase flex justify-between">
          ...
          {(contentType === "flashcards" || contentType === "both") && (
            <div className="space-y-3">
              <label className="text-[11px] font-mono font-bold tracking-widest text-zinc-400 uppercase flex justify-between">
                ...
                <span className="text-primary">{flashcardCount}</span>
              </label>
              <div className="flex items-center gap-4">
                <input
                  type="range"
...
// ─── Step 3 Component ────────────────────────────────────────────────────────
interface Step3CategoryFocusProps {
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="pt-4 border-t border-zinc-900">
        ...
        {categoryFocus === "new" && (
          <div className="flex flex-col gap-2">
            <label htmlFor="new-category-input" className="text-xs font-mono font-bold tracking-wider text-white uppercase">
            ...
            <input
              ...
              className="w-full bg-[#07080a] border border-border rounded-none px-4 py-2.5 text-sm font-mono text-white placeholder:text-zinc-700 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary min-h-[40px]"
            />
            <p className="text-[10px] font-mono text-zinc-500 uppercase mt-1">
              Slug will be auto-generated: <span className="text-primary">{newCategoryName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "---"}</span>
            </p>
          </div>
        )}
      </div>
...
export function Step4CopyPrompt({
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div className="space-y-1">
        <span className="text-[10px] font-mono tracking-widest text-[#4ae176] uppercase font-bold">
          STEP 04 // EXPORT_PROMPT
        </span>
        <h3 className="text-lg font-bold font-mono text-white tracking-tight">
          Copy Generation Prompt
        </h3>
        <p className="text-xs text-zinc-400 leading-relaxed font-sans">
          Your custom prompt has been compiled. Copy it below and paste it into any powerful LLM (e.g. Claude, GPT-4, Gemini) to generate the study material.
        </p>
      </div>

      <div className="relative border border-border bg-[#101115] rounded-none group">
        <div className="absolute top-3 right-3 z-10 flex gap-2">
          <button
            onClick={onCopyPrompt}
            className={cn(
              "text-[10px] font-mono font-bold uppercase tracking-wider px-3 py-1.5 border transition-all duration-150 cursor-pointer flex items-center gap-1.5",
              promptCopied
...
// ─── Step 5 Component ────────────────────────────────────────────────────────
interface Step5PasteMergeProps {
  jsonInput: string
