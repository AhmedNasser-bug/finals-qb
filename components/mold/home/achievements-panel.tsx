...
export function AchievementsPanel({
  ...
  return (
    <section className="bg-panel border border-border p-6 rounded-md">
      ...
      <div className="flex flex-col items-center py-5 bg-secondary/40 border border-border/60 rounded mb-6 relative overflow-hidden select-none">
        ...
        {topAchievements.map((ach) => {
          ...
          return (
            <div 
              key={ach.id}
              className={cn(
                "p-3.5 bg-card border flex items-center gap-3.5 transition-all duration-200",
                isUnlocked 
                  ? "border-primary/30 text-foreground" 
                  : "border-border text-muted-foreground opacity-40 grayscale"
              )}
            >
              <div className={cn("w-8 h-8 flex items-center justify-center border shrink-0 text-sm font-bold", isUnlocked ? "border-primary/20 bg-primary/5 text-primary" : "border-border bg-secondary/50")}>
