...
export function AchievementsPanel({
  ...
  return (
    <section className="bg-panel border border-border p-6 rounded-md">
      <div className="flex items-center gap-2 mb-6">
        <Trophy className="w-4 h-4 text-primary shrink-0" aria-hidden="true" />
        <h2 className="font-mono text-[10px] font-bold tracking-[0.2em] text-foreground/80">
          ACHIEVED_OBJECTIVES
        </h2>
      </div>

      <div className="flex flex-col items-center py-5 bg-secondary/50 border border-border/60 rounded mb-6 relative overflow-hidden select-none">
        <div className="text-5xl font-headline font-black text-foreground/10 relative tracking-wider">
        ...
        {topAchievements.map((ach) => {
          ...
          return (
            <div 
              key={ach.id}
              className={cn(
                "p-3.5 bg-secondary border flex items-center gap-3.5 transition-all duration-200",
                isUnlocked 
                  ? "border-primary/30 text-foreground"
                  : "border-border/80 text-muted-foreground opacity-40 grayscale"
              )}
            >
              <div className={cn("w-8 h-8 flex items-center justify-center border shrink-0 text-sm font-bold", isUnlocked ? "border-primary/20 bg-primary/5 text-primary" : "border-border/80 bg-secondary")}>
              ...
              <div className="min-w-0">
                ...
                <div className="font-sans text-[8px] text-muted-foreground/80 truncate leading-none mt-0.5">{ach.description}</div>
              </div>
            </div>
          )
        })}
