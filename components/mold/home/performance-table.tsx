...
export function PerformanceTable({
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        {runs.length > 0 && (
          <div
            ...
            role="group"
            aria-label="Filter runs by game mode"
          >
            <button
      ...
      {runs.length === 0 ? (
        <div
          className="flex flex-col items-center justify-center gap-2 py-12 rounded border border-dashed border-border/60 text-muted-foreground"
          role="status"
          aria-live="polite"
        >
        ...
        <div
          className="flex flex-col items-center justify-center gap-2 py-8 rounded border border-dashed border-border/60 text-muted-foreground"
          role="status"
          aria-live="polite"
        >
            ...
            <table className="w-full text-sm" aria-label="Recent runs">
              <thead>
                <tr className="border-b border-border bg-secondary/40">
                  <th className="text-left px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider">
                    DATE
                  </th>
                  <th className="text-left px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider">
                    MODE
                  </th>
                  <th className="text-right px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider">
                    SCORE
                  </th>
                  <th className="text-right px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider hidden sm:table-cell">
                    ANSWERS
                  </th>
                  <th className="text-right px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider hidden sm:table-cell">
                    TIME
                  </th>
                  <th className="text-right px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider hidden md:table-cell">
                    STREAK
                  </th>
                  <th className="text-center px-4 py-2.5 text-xs font-mono text-muted-foreground tracking-wider">
                    GRADE
                  </th>
                </tr>
              </thead>
          ...
          {filteredRuns.length > 5 && (
            <div className="p-2 border-t border-border bg-secondary/20 flex justify-center">
              <button
                ...
                className="text-xs font-mono text-primary hover:underline focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary px-2 py-1 rounded"
              >
                {showAllRuns
                  ? `SHOW RECENT (5)`
                  : `VIEW ALL (${filteredRuns.length} RUNS)`}
              </button>
            </div>
          )}
        </div>
