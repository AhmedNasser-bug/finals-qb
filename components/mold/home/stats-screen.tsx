  ...
  return (
    <div className="space-y-8 select-none animate-fade-in pb-16">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* Total Runs Card */}
        <div className="p-5 border border-border bg-panel rounded relative overflow-hidden group hover:border-border transition-all duration-300">
          <div className="absolute top-4 right-4 text-muted-foreground/30 group-hover:text-muted-foreground/50 transition-colors">
          ...
          <div className="mt-3 flex items-center gap-1.5 font-mono text-[9px] text-primary bg-primary/5 border border-primary/10 px-2 py-1 rounded w-fit">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="border border-border bg-panel rounded p-6 divide-y divide-zinc-800/60">
              {missions.map((mission) => (
                <div key={mission.id} className="py-4 first:pt-0 last:pb-0 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Progress slider */}
                    <div className="mt-2.5 w-full h-1 bg-surface-variant rounded overflow-hidden">
                      <div 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {milestones.map((milestone) => (
                <div 
                  key={milestone.id} 
                  className={cn(
                    "p-4 border rounded bg-panel relative overflow-hidden transition-all duration-300",
                    milestone.completed 
                  ...
                  <div className="flex items-start justify-between gap-2 relative z-10">
                    ...
                    <div className={cn(
                      ...
                      milestone.completed 
                        ...
                        : "bg-surface-variant/40 text-muted-foreground border border-border/60"
                    )}>
                      {milestone.completed ? "★" : `${Math.round((milestone.current / milestone.target) * 100)}%`}
                    </div>
                  </div>

                  <div className="mt-4 w-full h-1 bg-surface-variant/80 rounded overflow-hidden relative z-10">
                    <div 
                      className={cn(
            ...
            <div className="border border-destructive/20 bg-destructive/5 rounded p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              {!showConfirmReset ? (
                <button 
                  ...
                  className="bg-transparent hover:bg-destructive/10 text-destructive hover:text-destructive-foreground border border-destructive/30 px-4 py-2 rounded text-xs font-mono tracking-wider cursor-pointer transition-all shrink-0 uppercase font-bold"
                >
                   RESET_DATA
                </button>
              ) : (
                <div className="flex items-center gap-2 shrink-0">
                  <button 
                    ...
                    className="bg-destructive text-foreground hover:bg-destructive/90 px-3.5 py-2 rounded text-xs font-mono font-bold cursor-pointer transition-all uppercase"
                  >
                    CONFIRM
                  </button>
                  <button 
                    ...
                    className="bg-surface hover:bg-surface-variant border border-border text-foreground/90 px-3.5 py-2 rounded text-xs font-mono font-bold cursor-pointer transition-all uppercase"
                  >
                    CANCEL
                  </button>
                </div>
