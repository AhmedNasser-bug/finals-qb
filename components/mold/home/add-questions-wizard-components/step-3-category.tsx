...
export function Step3Category({ state, updateState, activeSubject }: Step3CategoryProps) {
  const { categoryFocus, selectedCategory, newCategoryName } = state
        ...
        {[
          ...
          return (
            <button
              key={focus.id}
              type="button"
              onClick={() => updateState({ categoryFocus: focus.id as any })}
              className={cn(
        ...
        {categoryFocus === "new" && (
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              onChange={(e) => updateState({ newCategoryName: e.target.value })}
