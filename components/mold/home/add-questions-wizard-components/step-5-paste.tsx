...
interface Step5PasteProps {
  state: WizardState
  handleJsonChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void
}

export function Step5Paste({ state, handleJsonChange }: Step5PasteProps) {
  const { jsonInput, validationState, validationResult, parsedPreview } = state
