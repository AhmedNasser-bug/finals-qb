...
export function Step4Prompt({ state, updateState, compiledPrompt }: Step4PromptProps) {
  const { promptCopied } = state

  function handleCopyPrompt() {
    navigator.clipboard.writeText(compiledPrompt)
    updateState({ promptCopied: true })
    setTimeout(() => {
      updateState({ promptCopied: false })
    }, 2000)
  }
