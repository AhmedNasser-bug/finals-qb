...
export function Step1Content({ state, updateState }: Step1ContentProps) {
  const { contentType, questionCount, flashcardCount } = state
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-border/60 bg-[#101115] p-5">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                step={5}
                value={questionCount}
                onChange={(e) => updateState({ questionCount: parseInt(e.target.value) || 10 })}
              ...
              <input
                ...
                max={50}
                value={questionCount}
                onChange={(e) => updateState({ questionCount: Math.min(50, Math.max(5, parseInt(e.target.value) || 0)) })}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-border/60 bg-[#101115] p-5">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                onChange={(e) => updateState({ flashcardCount: parseInt(e.target.value) || 10 })}
                className="w-40 accent-primary cursor-pointer"
              />
              <input
                ...
                max={50}
                value={flashcardCount}
                onChange={(e) => updateState({ flashcardCount: Math.min(50, Math.max(5, parseInt(e.target.value) || 0)) })}
                className="w-16 bg-[#07080a] border border-border rounded-none py-1.5 text-center text-sm font-mono text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
              />
            </div>
          </div>
