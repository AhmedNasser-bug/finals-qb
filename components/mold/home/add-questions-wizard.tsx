...
export function AddQuestionsWizard({
  ...
  const contentType = wizardState.contentType;
  const questionCount = wizardState.questionCount;
  const flashcardCount = wizardState.flashcardCount;
  const styleBias = wizardState.styleBias;
  const categoryFocus = wizardState.categoryFocus;
  const selectedCategory = wizardState.selectedCategory;
  const newCategoryName = wizardState.newCategoryName;
  const promptCopied = wizardState.promptCopied;
  const jsonInput = wizardState.jsonInput;
  const validationState = wizardState.validationState;
  const validationResult = wizardState.validationResult;
  const parsedPreview = wizardState.parsedPreview;

  const setStep = (val: number | ((prev: number) => number)) => {
    setWizardState((prev) => ({
      ...prev,
      step: typeof val === "function" ? val(prev.step) : val,
    }));
  };
  const setJsonInput = (val: string) => updateState({ jsonInput: val });
  const setValidationState = (val: any) =>
    updateState({ validationState: val });
  const setValidationResult = (val: any) =>
    updateState({ validationResult: val });
  const setParsedPreview = (val: any) => updateState({ parsedPreview: val });
    ...
    <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in select-none">
      <div className="w-full max-w-5xl h-[88vh] flex flex-col gap-0 border border-border bg-[#0a0b0d] rounded-none overflow-hidden border-glow transition-all duration-300">
        ...
        <div className="flex-1 px-8 py-6 overflow-y-auto min-h-0 bg-[#0a0b0d]">
          {step === 1 && (
            <Step1Content state={wizardState} updateState={updateState} />
          )}
          {step === 2 && (
            <Step2Style state={wizardState} updateState={updateState} />
          )}
          {step === 3 && (
            <Step3Category
              state={wizardState}
