...
export function AddQuestionsWizard({
  ...
  const [contentType, setContentType] = useState<
    "questions" | "flashcards" | "both"
  >("questions");
  const [questionCount, setQuestionCount] = useState<number>(15);
  const [flashcardCount, setFlashcardCount] = useState<number>(15);

  // Step 2: Style Bias
  const [styleBias, setStyleBias] = useState<
    "theoretical" | "technical" | "balanced"
  >("balanced");
  ...
  const [parsedPreview, setParsedPreview] = useState<FullSubjectData | null>(
    null,
  );
  ...
  const compiledPrompt = useMemo(() => {
    ...
    if (contentType === "questions" || contentType === "both") {
      itemsToGenerate.push(
        `EXACTLY ${questionCount} unique, high-quality questions (MCQ or TrueFalse format)`,
      );
    }
  ...
  const handleConfirmMerge = () => {
    ...
    Object.entries(incomingTerminology).forEach(([cat, entries]) => {
      ...
      const existingTerms = new Set(
        existingTerminology[cat].map((e) => e.term.toLowerCase().trim()),
      );
    ...
    const existingCategoriesOrder = Array.from(
      new Set(existingQuestions.map((q) => q.category)),
    );
    ...
    combinedQuestions.sort(
      (a, b) => catIndex(a.category) - catIndex(b.category),
    );
    combinedFlashcards.sort(
      (a, b) => catIndex(a.category) - catIndex(b.category),
    );
            ...
            <div className="space-y-6 animate-slide-up">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Select the types of learning materials you would like the AI
                  to generate and configure their quantities.
                </p>
              </div>
                ...
                {[
                  {
                    id: "questions",
                    name: "Questions Only",
                    desc: "MCQ & TrueFalse sets",
                  },
                  {
                    id: "flashcards",
                    name: "Flashcards Only",
                    desc: "Key concept definitions",
                  },
                  {
                    id: "both",
                    name: "Both Sets",
                    desc: "Unified study collection",
                  },
                ].map((type) => {
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-border/60 bg-[#101115] p-5">
                    <div className="flex flex-col">
                      <label
                        ...
                        className="text-xs font-mono font-bold tracking-wider text-white uppercase"
                      >
                        Questions Count
                      </label>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setQuestionCount(
                            Math.min(
                              50,
                              Math.max(5, parseInt(e.target.value) || 0),
                            ),
                          )
                        }
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-border/60 bg-[#101115] p-5">
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onChange={(e) =>
                          setFlashcardCount(parseInt(e.target.value) || 10)
                        }
                        className="w-40 accent-primary cursor-pointer"
                      />
                      <input
          ...
          {step === 2 && (
            <div className="space-y-6 animate-slide-up">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Select the structural flavor of the questions. Technical
                  profiles generate code elements, trace data, and visual state
                  diagrams, while theoretical sets focus on core logic and
                  terms.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-zinc-400 leading-normal font-sans">
                      Ideal for definitions, logical relationships, historical
                      context, and explaining "how" and "why" through
                      explanatory prose.
                    </p>
                  </div>
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-zinc-400 leading-normal font-sans">
                      Instructs the AI to embed programming syntax, HTML trace
                      tables, calculations, and Mermaid flowcharts or sequence
                      diagrams.
                    </p>
                  </div>
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <p className="text-[11px] text-zinc-400 leading-normal font-sans">
                      A uniform mixture of algorithmic application, code blocks,
                      visual flowcharts, and theoretical/prose concepts.
                    </p>
                  </div>
                  <span className="text-[9px] font-mono tracking-widest uppercase font-bold block border-t border-border/30 pt-2 text-primary">
          ...
          {step === 3 && (
            <div className="space-y-6 animate-slide-up">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Choose where the new items belong. You can target all existing
                  categories, target a specific existing one, or create a brand
                  new category.
                </p>
              </div>
                ...
                {[
                  {
                    id: "all",
                    name: "All Categories",
                    desc: "Distribute generated items across current categories",
                  },
                  {
                    id: "existing",
                    name: "Existing Category",
                    desc: "Focus strictly on a single category selected below",
                  },
                  {
                    id: "new",
                    name: "New Category",
                    desc: "Create a brand new category slug and display name",
                  },
                ].map((focus) => {
                ...
                {categoryFocus === "all" && (
                  <div className="p-4 bg-zinc-900/40 border border-border/50 text-xs font-mono text-zinc-400">
                    <p className="font-bold text-white uppercase mb-1">
                      Target Categories Context:
                    </p>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {existingCategories.map((cat) => (
                        <span
                          key={cat}
                          className="px-2 py-0.5 bg-zinc-800 text-zinc-300 text-[10px] border border-zinc-700/50"
                        >
                          {formatLabel(cat)}
                        </span>
                      ))}
          ...
          {step === 4 && (
            <div className="space-y-6 animate-slide-up">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Below is the dynamically constructed prompt guiding the AI to
                  output valid, structured questions that match your choices.
                  Copy this prompt and paste it into Gemini, Claude, or ChatGPT.
                </p>
              </div>
              ...
              <div className="p-4 border border-primary/20 bg-primary/5 rounded-none flex items-start gap-3">
                ...
                <div className="text-xs leading-relaxed font-sans text-zinc-300">
                  ...
                  <ol className="list-decimal list-inside space-y-1 mt-1 text-zinc-400">
                    ...
                    <li>
                      Paste it into your favorite LLM (Claude-3.5-Sonnet or
                      Gemini-1.5-Pro recommended).
                    </li>
                    <li>Wait for the LLM to output the raw JSON block.</li>
                    <li>
                      Copy the JSON and click <b>CONTINUE</b> to paste and
                      merge.
                    </li>
                  </ol>
                </div>
          ...
          {step === 5 && (
            <div className="space-y-6 animate-slide-up">
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">
                  Paste the generated JSON block here. We will validate it,
                  automatically repair syntax/escape flaws, and preview the
                  parsed questions and flashcards before merging.
                </p>
              </div>

              <div className="flex flex-col gap-2">
                <label
                  htmlFor="ai-json-input"
                  className="text-xs font-mono font-bold tracking-wider text-zinc-400 uppercase"
                >
                ...
                <textarea
                  ...
                  className={cn(
                    ...
                    validationState === "valid"
                      ? "border-emerald-500/50 focus-visible:ring-1 focus-visible:ring-emerald-500"
                      : validationState === "error"
                        ? "border-red-500/50 focus-visible:ring-1 focus-visible:ring-red-500"
                        ...
                        <div className="grid grid-cols-3 gap-4 bg-zinc-950/50 p-4 border border-zinc-900 text-zinc-300 font-mono text-[11px]">
                          <div>
                            ...
                            <span className="text-lg font-bold text-white">
                              {
                                parsedPreview.questions.filter(
                                  (q) => q.id !== "q-default-1",
                                ).length
                              }
                            </span>
                          ...
                          <div>
                            ...
                            <span className="text-xs font-bold text-white truncate block">
                              {Array.from(
                                new Set(
                                  parsedPreview.questions
                                    .filter((q) => q.id !== "q-default-1")
                                    .map((q) => q.category),
                                ),
                              )
