...
const PII_PATTERNS: Array<{ pattern: RegExp; replacement: RedactReplacement }> = [
  ...
  },
  {
    ...
    pattern: /([a-zA-Z0-9_]*(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)["']?\s{0,10}[:=]\s{0,10})(?:(")([^"]{0,4096})(")|(')([^']{0,4096})(')|([^,\]\}\s]{1,4096}))/gi,
...
const SENSITIVE_KEYS_REGEX = /[a-zA-Z0-9_]*(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)/i;
