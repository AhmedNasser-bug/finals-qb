...
const PII_PATTERNS: Array<{ pattern: RegExp; replacement: RedactReplacement }> = [
  {
    // Emails
    pattern: /([a-zA-Z0-9._%+-]{1,255}@[a-zA-Z0-9.-]{1,255}\.[a-zA-Z]{2,63})/g,
    replacement: '[REDACTED]'
  },
  {
    // Bearer tokens
    pattern: /(Bearer\s+)([A-Za-z0-9\-\._~\+\/]{1,4096}={0,3})/g,
    replacement: '$1[REDACTED]'
  },
  {
    // Private keys
    pattern: /(-----BEGIN[A-Z0-9-\s]{0,255}PRIVATE KEY-----)([\s\S]{1,8192}?)(-----END[A-Z0-9-\s]{0,255}PRIVATE KEY-----)/g,
    replacement: '$1\n[REDACTED]\n$3'
  },
  {
    // Common secrets and PII
    pattern: /((?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)["']?\s*[:=]\s*)(?:(")([^"]{0,4096})(")|(')([^']{0,4096})(')|([^,\]\}\s]{1,4096}))/gi,
