...
const PII_PATTERNS: Array<{ pattern: RegExp; replacement: RedactReplacement }> = [
  ...
  },
  {
    ...
    replacement: (match: string, bnd: string, p1: string, p2: string, p3: string, p4: string, p5: string, p6: string, p7: string, p8: string) => {
      if (p8 && (p8.startsWith('[') || p8.startsWith('{'))) {
        return match;
      }
      if (p2) return bnd + p1 + p2 + "[REDACTED]" + p4;
      if (p5) return bnd + p1 + p5 + "[REDACTED]" + p7;
      return bnd + p1 + '"[REDACTED]"';
    }
  },
  {
    // JWTs
