...
const PII_PATTERNS: Array<{ pattern: RegExp; replacement: RedactReplacement }> = [
  ...
  {
    // Common secrets and PII (Double quotes)
    pattern: /(\b[a-zA-Z0-9_]*(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)[a-zA-Z0-9_]*["']?\s{0,10}[:=]\s{0,10}")([^"]*)(")/gi,
    replacement: '$1[REDACTED]$3'
  },
  {
    // Common secrets and PII (Single quotes)
    pattern: /(\b[a-zA-Z0-9_]*(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)[a-zA-Z0-9_]*["']?\s{0,10}[:=]\s{0,10}')([^']*)(')/gi,
    replacement: '$1[REDACTED]$3'
  },
  {
    // Common secrets and PII (Unquoted)
