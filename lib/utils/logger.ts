...
const PII_PATTERNS: Array<{ pattern: RegExp; replacement: RedactReplacement }> = [
  {
    // Emails
    pattern: /([a-zA-Z0-9._%+-]{1,255}@[a-zA-Z0-9.-]{1,255}\.[a-zA-Z]{2,64})/g,
    replacement: '[REDACTED]'
  },
  {
    // Bearer tokens
    pattern: /(Bearer\s{1,10})([A-Za-z0-9\-\._~\+\/]{1,4096}={0,2})/g,
  ...
  },
  {
    ...
    pattern: /([a-zA-Z0-9_.-]*?(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)[a-zA-Z0-9_.-]*?["']?\s{0,10}[:=]\s{0,10})(?:(")([^"]{0,4096})(")|(')([^']{0,4096})(')|([^,\]\}\s]{1,4096}))/gi,
...
const SENSITIVE_KEYS_REGEX = /[a-zA-Z0-9_.-]*?(?:api_key|apikey|secret|token|password|email|phone|ssn|credit_card)[a-zA-Z0-9_.-]*/i;
