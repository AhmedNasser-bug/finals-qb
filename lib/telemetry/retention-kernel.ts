...
export function deriveCategoryRetentionSummaries(
  ...
  return Object.entries(groups).map(([cat, cardStates]) => {
    ...
    for (const s of cardStates) {
      if (s.urgencyLevel === "MASTERED") {
        mastered++
      } else if (s.urgencyLevel === "DUE" || s.urgencyLevel === "APPROACHING_DECAY") {
        due++
      } else if (s.urgencyLevel === "CRITICAL_LAPSED") {
        lapsed++
      }
      totalRetrievability += s.currentRetrievability
      totalStability += s.stability
    }

    const avgR =
      total > 0
        ? Math.round((totalRetrievability / total) * 100) / 100
        : 0
    const avgS =
      total > 0
        ? Math.round((totalStability / total) * 10) / 10
        : 0
