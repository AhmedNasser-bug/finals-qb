...
export function analyzeCognitiveLoad(
  ...
  if (latencySamples.length >= 4) {
    ...
    let sum1 = 0
    for (let i = 0; i < mid; i++) {
      sum1 += latencySamples[i]
    }
    const avg1 = sum1 / mid

    let sum2 = 0
    const secondHalfLen = latencySamples.length - mid
    for (let i = mid; i < latencySamples.length; i++) {
      sum2 += latencySamples[i]
    }
    const avg2 = sum2 / secondHalfLen
