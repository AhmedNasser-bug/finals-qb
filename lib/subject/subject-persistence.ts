...
function autoFixAnswer(qObj: Record<string, unknown>, normalizedOptions: any[], warnings: string[], i: number): boolean {
...
function autoFixSingleQuestion(
  ...
  if (autoFixAnswer(qObj, normalizedOptions, warnings, i)) {
    qFixed = true;
  }
...
function validateSingleQuestion(
  ...
  const hasLabel = (qObj.options as Record<string, unknown>[]).some((opt) => opt.label === qObj.answer);
