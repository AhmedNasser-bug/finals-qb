...
function autoFixAnswer(
  ...
  const hasLabel = normalizedOptions.some(opt => opt.label === qObj.answer);
  if (hasLabel) {
    return false;
  }

  const matchedOpt = normalizedOptions.find(opt => opt.text.toUpperCase() === qObj.answer);
  if (matchedOpt) {
...
function autoFixSingleQuestion(
  ...
  const answerFixed = autoFixAnswer(qObj, normalizedOptions, i, warnings);
  if (answerFixed) {
    qFixed = true;
  }
