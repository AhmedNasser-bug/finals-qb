...
## 2024-06-28 - Avoid O(N) array operations like lastIndexOf and splice
**Learning:** Using O(N) `.lastIndexOf()` and `.splice()` together in a loop to pop to a matched stack element results in O(N^2) overhead without allocating an array copy.
**Action:** Replace `.lastIndexOf() + .splice()` behavior with a native backward `while` loop combined with `.length = idx` truncation to emulate slicing without allocations.
