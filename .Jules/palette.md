...
## 2024-07-04 - Accessible Context for Generic Layout Elements
**Learning:** Decorative icons placed next to explicit text labels (e.g. navigation buttons) cause redundant screen reader announcements if left unhidden. Similarly, custom dynamic state transitions (like reset confirmations) need `aria-live` to announce themselves without focus movement, and visually-complex visual data (like pixel grids) need a descriptive `aria-label` and `role="img"` to communicate intent instead of bombarding users with 100 individual unlabelled boxes.
**Action:** Always add `aria-hidden="true"` to generic icons within buttons that contain labels. Wrap custom prompt elements in `role="status"` and `aria-live="polite"`. Consolidate visual grids with a single `aria-label` and `role="img"` wrapper.
