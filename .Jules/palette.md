...
## 2026-05-25 - Semantic Mobile Navigation Landmarks
**Learning:** Primary mobile navigation bars (like a bottom tab bar) are often incorrectly implemented with generic `<footer>` tags, which misrepresents their purpose to assistive tech. Also, heavily abbreviated visual labels (like "ACHS") fail to communicate clear intent.
**Action:** Always use a `<nav>` landmark for primary navigation bars (e.g., `<nav aria-label="Main mobile navigation">`). Additionally, add explicit `aria-label` and `title` attributes to buttons containing heavily abbreviated visual labels, and mark their purely decorative icons as `aria-hidden="true"`.
