...
## 2026-06-13 - Focus Traps and Accessible Custom Interactive Elements
**Learning:** Modal focus traps require explicit `tabIndex={-1}` on the dialog wrapper and explicit focus handling on mount. Furthermore, wrapping complex interactive layouts in a top-level native `<button>` violates HTML validity if inner elements are also interactive; replacing them with an accessible `<div role="button" tabIndex={0}>` accompanied by keyboard handlers preserves layout while passing semantic checks.
**Action:** Audit modals to ensure explicit focus traps and `tabIndex` configurations are in place. Always avoid nesting interactive elements inside `<button>` tags; use accessible `<div role="button">` constructs for complex clickable layouts.
