...
## 2026-06-18 - Add explicit nav landmarks and contextual attributes to bottom mobile navigation
**Learning:** Replaced the semantic-less `<footer>` wrapper with a `<nav aria-label="Main mobile navigation">`, establishing a standard landmark for assistive technologies. Additionally, action buttons that rely on highly abbreviated labels (e.g., 'ACHS', 'DATA') or icon-only identifiers severely lack context for screen reader and keyboard users. Applying `title` attributes solves this gap, and explicitly decorating contextual SVG icons with `aria-hidden="true"` prevents redundant announcements when readable text is already provided in the element.
**Action:** Always wrap primary mobile application menus in a `<nav>` with an `aria-label`. Ensure abbreviated text actions are given full `title` definitions, and use `aria-hidden="true"` on decorative icons inside labeled elements.
