...
## 2026-06-10 - Adding context to purely stylistic textual buttons
**Learning:** Buttons with stylised acronyms or very brief text (e.g., 'CORE', 'DATA', 'ACHS') often fail to communicate their intent clearly to screen readers compared to visual users who have surrounding context.
**Action:** Add descriptive `title` and `aria-label` attributes to translate stylistic text into clear actions (e.g., `aria-label="Go to Core Dashboard"`) without compromising the visual design, and adding `aria-hidden="true"` to the SVGs for purely decorative, secondary icons inside buttons.
