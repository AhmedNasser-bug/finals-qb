...
## 2024-06-14 - Mobile Nav Accessibility & UX\n\n**Learning:** Mobile bottom navigation bars often rely on highly abbreviated labels (e.g., "ACHS", "DATA") to save space, which can be confusing for screen reader users and new users. Additionally, using a `<footer>` tag instead of a `<nav>` tag for main app navigation misrepresents the landmark region.\n**Action:** Always use a `<nav aria-label="...">` landmark for primary navigation. Provide full, descriptive `aria-label` and `title` attributes for abbreviated buttons, and use `aria-current="page"` to indicate active state for screen readers.\n
