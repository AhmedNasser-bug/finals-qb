...
## 2026-06-26 - [Mobile Bottom Navigation Accessibility]
**Learning:** In a highly stylized, dense UI (like the mobile bottom navigation bar), abbreviated text labels (e.g. CORE, STATS) under icons might not be descriptive enough for screen reader users or recognizable for new users relying on tooltips. Additionally, finding duplicated components (like `MobileBottomNavBar` appearing in multiple files) means accessibility fixes must be carefully applied across all instances to prevent inconsistent UX states.
**Action:** Always implement explicit `aria-label` and `title` attributes on navigation icon buttons, even when they have abbreviated companion text. Ensure that when auditing components for a11y improvements, check for duplicated or similarly-named exported components to ensure the fix is comprehensive across the entire codebase.
