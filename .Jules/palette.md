...
## 2026-06-18 - [Accessible Mode Selectors and Action Buttons]
**Learning:** Found several icon buttons and custom UI components missing appropriate ARIA labels and focus states. Especially with custom styled interactive elements, standardizing `focus-visible:ring-1` and `aria-label` ensures components are perceivable to screen reader users.
**Action:** Applied `aria-label` and explicit `focus-visible` utility classes across nav-bars, mode selectors, and confirmation dialogs.
