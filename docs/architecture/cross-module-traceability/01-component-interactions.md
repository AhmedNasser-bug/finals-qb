...
### Interaction Flow
1. **Initial Render**: Next.js server components render the skeleton.
2. **Client Interaction**: The user interacts with Client Components. Contexts from `lib/game-engine.tsx` and stores like `lib/active-subject-store.ts` mutate based on actions.
3. **State Syncing**: Persistent states sync down into `localStorage` leveraging persistence controllers.
