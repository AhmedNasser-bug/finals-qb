...
2. **Bootstrap the Environment:**
   Run the idempotent orchestration script to seed mock data, install dependencies if necessary, and spin up Docker containers for multi-tenant setups via Docker Compose:
   ```bash
...
3. **Launch the Server:**
   Start the development server using `pnpm` exclusively:
   ```bash
   pnpm dev
   ```
