...
- **Test everything:** Ensure **`pnpm test`** passes cleanly without regressions. Always execute unit tests immediately before the pre-commit phase to comply with the Completeness Rule.
- **Architectural boundaries:** Changes to monolithic UI files should isolate nested elements and use explicit interfaces. Any logic modified must align with `docs/architecture/cross-module-traceability/`.
