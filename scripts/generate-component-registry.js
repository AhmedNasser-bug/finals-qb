...
function extractInterface(content, componentName) {
  const declarationRegex = /(?:interface\s+\w+|type\s+\w+\s*=)\s*\{/;
  const match = content.match(declarationRegex);
  if (!match) return 'None specified or inline props';

  const startIndex = match.index + match[0].length - 1; // index of the opening brace '{'
  let depth = 0;
  let endIndex = -1;
